package xyz.WorstClient.jello.main.particles;
...
public class ParticleEngine {
	...
	public void render(float mouseX, float mouseY){
		...
		for(Particle p : particles){
			...
			RenderUtil.drawCircle(p.x + Math.sin(p.ticks/2)*50 + -xOffset/5, (p.ticks*p.speed)*p.ticks/10 + -yOffset/5, p.radius*(p.opacity/32), c.getRGB());
		...
		GlStateManager.enableBlend();
		GlStateManager.disableAlpha();
		lastMouseX = net.minecraft.client.gui.GuiSimpleScrolledSelectionListProxy.getMouseX();
		lastMouseY = net.minecraft.client.gui.GuiSimpleScrolledSelectionListProxy.getMouseY();
	}
