...
public class Module {
	...
	public void setEnabled(boolean enabled) {
		...
		if (enabled) {
			Client.instance.sendClientMessage(this.getName() + " Enabled", Notification.Type.SUCCESS);
			animation = 0;
			this.onEnable();

			EventBus.getInstance().register(this);
		} else {
			Client.instance.sendClientMessage(this.getName() + " Disable", Notification.Type.ERROR);
			...
			if (option != null) {
				...
				if (xd != null) {
					if (xd.isValid(args[1])) {
						xd.setMode(args[1]);
						Helper.sendMessage(
								String.format("> %s set to %s", new Object[] { xd.getName(), xd.getModeAsString() }));
					} else {
