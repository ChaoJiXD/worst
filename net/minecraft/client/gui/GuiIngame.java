...
import xyz.WorstClient.module.modules.render.Crosshair;
import xyz.WorstClient.module.modules.render.HUD;
import xyz.WorstClient.module.modules.render.SetScoreboard;
...
public class GuiIngame extends Gui
{
    ...
    {    	
		...
		UnicodeFontRenderer wqy = Client.fontManager.wqy20;
    	if(Client.getModuleManager().getModuleByClass(HUD.class).isEnabled()) {

	    
	   	RenderUtil.drawRect(0, sr.getScaledHeight() - 23, sr.getScaledWidth(), sr.getScaledHeight(), new Color(0,0,0,120).getRGB());


    	if(mc.thePlayer.inventory.currentItem == 0) {
	   		RenderUtil.drawRect(sr.getScaledWidth() / 2 - 92, sr.getScaledHeight() - 23, (sr.getScaledWidth() / 2 + 91) - 20 * 8, sr.getScaledHeight(),new Color(255,255,255,150).getRGB());
	   	}else if(mc.thePlayer.inventory.currentItem == 8) {
	   		RenderUtil.drawRect(sr.getScaledWidth() / 2 +68, sr.getScaledHeight() - 23, (sr.getScaledWidth() / 2 + 92), sr.getScaledHeight(),new Color(255,255,255,150).getRGB());
	   	}else {
	   		RenderUtil.drawRect((sr.getScaledWidth() / 2) - 92 + mc.thePlayer.inventory.currentItem * 20, sr.getScaledHeight() - 23, (sr.getScaledWidth() / 2) + 91 - 20 * (8 - mc.thePlayer.inventory.currentItem), sr.getScaledHeight(), new Color(255,255,255,150).getRGB());
	   	}
	   	GlStateManager.disableBlend();
	}
    ...
    {                
        ...
        if(Client.instance.getModuleManager().getModuleByClass(Crosshair.class).isEnabled()) {
            return false;
        }else {
            return true;
        }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        int k1 = (int) (p_180475_2_.getScaledHeight() / 2 + j1 / 3 + SetScoreboard.Y.getValue());
        byte b0 = 3;
        ...
        for (Object score1 : arraylist1)
        {
            ...
            if(Client.getModuleManager().getModuleByClass(SetScoreboard.class).isEnabled()) {
    ...
    {
    	UnicodeFontRenderer font= Client.fontManager.getFont("googlesans.ttf", 17.0f);
        ...
        if (itemstack != null)
        {
            ...
            this.itemRenderer.renderItemOverlays(font, itemstack, xPos, yPos);
        }
    }
