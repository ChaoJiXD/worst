...
public class ServerCommandManager extends CommandHandler implements IAdminCommand
{
    public ServerCommandManager()
    {
        ...
        if (MinecraftServers.getServer().isDedicatedServer())
        {
    ...
    public void notifyOperators(ICommandSender sender, ICommand command, int flags, String msgFormat, Object... msgParams)
    {
        ...
        if (flag)
        {
            for (EntityPlayer entityplayer : minecraftserver.getConfigurationManager().func_181057_v())
            {
                ...
                {
                    boolean flag1 = sender instanceof MinecraftServers && MinecraftServers.getServer().func_183002_r();
                    boolean flag2 = sender instanceof RConConsoleSource && MinecraftServers.getServer().func_181034_q();

                    if (flag1 || flag2 || !(sender instanceof RConConsoleSource) && !(sender instanceof MinecraftServers))
                    {
        ...
        if ((flags & 1) != 1 && flag3 || sender instanceof MinecraftServers)
        {
