...
public class CommandOp extends CommandBase
{
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos)
    {
        if (args.length == 1)
        {
            ...
            for (GameProfile gameprofile : MinecraftServers.getServer().getGameProfiles())
            {
                if (!MinecraftServers.getServer().getConfigurationManager().canSendCommands(gameprofile) && doesStringStartWith(s, gameprofile.getName()))
                {
