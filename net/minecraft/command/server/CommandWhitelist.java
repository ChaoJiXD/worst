    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos)
    {
        ...
        else
        {
            if (args.length == 2)
            {
                ...
                {
                    return getListOfStringsMatchingLastWord(args, MinecraftServers.getServer().getConfigurationManager().getWhitelistedPlayerNames());
                }

                if (args[0].equals("add"))
                {
                    return getListOfStringsMatchingLastWord(args, MinecraftServers.getServer().getPlayerProfileCache().getUsernames());
                }
            }
