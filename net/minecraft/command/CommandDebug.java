...
public class CommandDebug extends CommandBase
{
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException
    {
        ...
        else
        {
            if (args[0].equals("start"))
            {
                ...
                MinecraftServers.getServer().enableProfiling();
                this.field_147206_b = MinecraftServers.getCurrentTimeMillis();
                this.field_147207_c = MinecraftServers.getServer().getTickCounter();
            }
            else
            {
                ...
                if (!MinecraftServers.getServer().theProfiler.profilingEnabled)
                {
                    throw new CommandException("commands.debug.notStarted", new Object[0]);
                }

                long i = MinecraftServers.getCurrentTimeMillis();
                int j = MinecraftServers.getServer().getTickCounter();
                long k = i - this.field_147206_b;
                int l = j - this.field_147207_c;
                this.func_147205_a(k, l);
                MinecraftServers.getServer().theProfiler.profilingEnabled = false;
    ...
    {
        File file1 = new File(MinecraftServers.getServer().getFile("debug"), "profile-results-" + (new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss")).format(new Date()) + ".txt");
    ...
    {
        List<Profiler.Result> list = MinecraftServers.getServer().theProfiler.getProfilingData(p_147202_2_);
